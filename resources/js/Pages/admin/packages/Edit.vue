  ...
  <div class="page">
    <div class="head">
      <div>
        ...
        <div class="subtitle">Update package details, pricing, and availability.</div>
      </div>

      <div class="head-actions">
        <button class="btn btn-light" type="button" @click="goBack" :disabled="saving">Back</button>
        <button class="btn" type="button" @click="submit" :disabled="saving || loading">{{ saving ? 'Saving...' : 'Update Package' }}</button>
      </div>
    </div>
      ...
      <div class="card">
        <form @submit.prevent="submit">
          ...
          <div class="form-actions">
            <button class="btn" type="submit" :disabled="saving">{{ saving ? 'Saving...' : 'Update Package' }}</button>
          </div>
        </form>
      </div>
...
async function submit() {
  ...
  try {
    if (pickedFile.value) {
      ...
      await window.axios.post(`/api/admin/packages/${route.params.id}`, fd, {
        params: { _method: 'PUT' },
...
.loading{color:#64748b;padding:12px 0;font-weight:800;}
